...
- Host assets using HTTPS links that expose the filename extension (see `examples/upload-hosting.md`). Google Drive works as long as the direct link includes the original filename.
- Respect the `fileUrls`/`imageUrls` limits: at most 10 links per field. If you need more files, bundle them into ZIPs or reuse hosts.
- Full example: `examples/create-creation.graphql` mirrors the payload from `Create a design.graphql`, including `tagNames` and `metaTags`.

Each markdown file stays compact and is written to be LLM-friendly: clear headings, minimal cross-file dependencies, and short paragraphs that can be chunked independently.
...
| `examples/create-creation.graphql` | Ready-to-run payload with inline `fileUrls`/`imageUrls`. |
| `examples/upload-hosting.md` | Quick guide to generate direct Google Drive download links. |
| `field_matrix.md` | Field-level map for common objects (Sale, Order, Creation, User). |
| `data_dictionary.md` | Glossary of objects, fields, arguments, and sources. |
| `stability_matrix.md` | Field stability map (stable vs recent vs volatile). |
| `error_catalog.md` | Common API errors with likely causes and fixes. |
| `rate_limit_contract.md` | Throttling contract and backoff guidance. |
| `changelog.md` | Dated log of gist and Discord updates. |
...
| `schema/README.md` | How to capture the GraphQL schema snapshot locally. |
| `schema/introspection.graphql` | Introspection query used to pull the schema. |
| `scripts/` | Local helpers for schema capture + contract checks. |
...
Before shipping a client, verify field names in GraphiQL:
...
```
Optionally run `python scripts/contract_checks.py` to validate the fields listed in `stability_matrix.md`.
...
See a new snippet in `#api-help`? Add it to the relevant file with the date/source noted so everyone can trace provenance, and append it to `changelog.md`.
