...
Compress-Archive -Path '.\\*.md','.\workflows','.\examples','.\schema','.\scripts' -DestinationPath $Dest
```
- The resulting `cults-api-docs.zip` is the downloadable bundle requested for LLM loading.
- Verify that no credentials or spreadsheet IDs ended up inside the archive before sharing.

## Feeding a Model
1. **Chunk order** - start with `README.md`, then `architecture.md`, `data_dictionary.md`, `field_matrix.md`, and `stability_matrix.md`. After that, load the workflow that matches your question (creation/update/asset), then `endpoints.md` or `examples/*.md`, and finally `error_catalog.md` or `rate_limit_contract.md` for operational concerns.
